...
// FileConfig is the YAML shape for a server config file. Load
...
// previous pre-registry shape where modules lived at the root — `gateway:`,
...
func Load(path string) (server.Config, error) {
	...
	if err != nil {
		return server.Config{}, fmt.Errorf("server: read config %q: %w", path, err)
	}
	...
	if err := dec.Decode(&fc); err != nil {
		return server.Config{}, translateTopLevelYAMLError(err, path)
	}
	return fc.toConfig()
}
...
// pointer to the modules: map when the offending key is a previous
...
func (fc FileConfig) toConfig() (server.Config, error) {
	cfg := server.Config{
		Namespace: fc.Namespace,
		FSRoot:    fc.FSRoot,
		SecretKey: fc.SecretKey,
	}
	var openedStore brainkit.KitStore
	success := false
	defer func() {
		if !success && openedStore != nil {
			_ = openedStore.Close()
		}
	}()

	storePath := fc.KitStorePath
	if storePath == "" && fc.FSRoot != "" {
	...
	for _, name := range names {
		factory, ok := bkmodule.Lookup(name)
		if !ok {
			return server.Config{}, unknownModuleError(name)
		}
		...
		if err != nil {
			return server.Config{}, fmt.Errorf("server: module %q: %w", name, err)
		}
		if mod != nil {
