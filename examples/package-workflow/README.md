...
1. **Scaffold**. `packagescaffold.ScaffoldPackage(dir, name, entry, source)`
...
2. **Deploy from directory**. `packageclient.FromDir(dir)` returns
   a `Package` the packages module can install. `packageclient.Deploy(ctx, kit, pkg)` reads
...
3. **Edit + redeploy**. Rewrite the entry file on disk, call
   `packageclient.Deploy` again with the same `Package`. brainkit
...
5. **Teardown**. `packageclient.Teardown(ctx, kit, "greeter")` removes the
...
[1/5] scaffolding package at /tmp/package-workflow-demo
          ...
          types/agent.d.ts …
[2/5] deploying packageclient.FromDir(...)
...
[3/5] editing index.ts on disk (adding a 'facts' handler)
        ...
        ts.greeter.facts reply:
          ...
          • packageclient.Deploy(packageclient.FromDir(path)) ships whatever's on disk.
...
err := packagescaffold.ScaffoldPackage(
...
```go
packagescaffold.ScaffoldPackage(dir, name, entry, source, packagescaffold.ScaffoldOptions{
    Version:     "1.2.0",
...
| Helper | When |
|---|---|
| `packageclient.Inline(name, entry, source)` | One-off deploys, tests, examples where the source is a literal string in the Go file. No disk layout. |
| `packageclient.FromFile(path)` | You have a single `.ts` file on disk but don't need the full manifest + types shape. Manifest is synthesized from the filename. |
| `packageclient.FromDir(dir)` | You have a full scaffold on disk (this example). Manifest + tsconfig + types/ already exist. The shape `brainkit new package` produces. |

## Under the hood

- The Go `ScaffoldPackage` helper is the exact code the CLI's
  `brainkit new package` runs. One source of truth.
- The embedded `.d.ts` content is written by `packagescaffold.ScaffoldPackage`
