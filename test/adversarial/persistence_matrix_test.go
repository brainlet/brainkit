...
// P7: WASM module survives restart
func TestPersistence_WASMModule(t *testing.T) {
	withRestart(t,
		func(k *brainkit.Kernel) {
			ctx := context.Background()
			// Compile a simple AS module
			pr, err := sdk.Publish(k, ctx, messages.WasmCompileMsg{
				Source: `export function run(): i32 { return 42; }`,
				Options: &messages.WasmCompileOpts{Name: "persist-mod"},
			})
			require.NoError(t, err)
			...
			select {
			case p := <-ch:
				assert.Contains(t, string(p), "persist-mod")
			case <-time.After(30 * time.Second):
				t.Fatal("timeout compiling WASM module")
			}
		},
		func(k *brainkit.Kernel) {
			// Module should be available after restart
			modules, err := k.ListWASMModules()
			require.NoError(t, err)
			found := false
			for _, m := range modules {
				if m.Name == "persist-mod" {
					found = true
				}
			}
			assert.True(t, found, "WASM module should survive restart")
		},
	)
}

// P9: Persistent shard state survives restart
func TestPersistence_ShardState(t *testing.T) {
	tmpDir := t.TempDir()
	storePath := filepath.Join(tmpDir, "store.db")

	// Phase 1: Compile module, deploy shard, set state
	store1, err := brainkit.NewSQLiteStore(storePath)
	require.NoError(t, err)
	k1, err := brainkit.NewKernel(brainkit.KernelConfig{
		Namespace: "test", CallerID: "test", FSRoot: tmpDir,
	...
	// Compile — use brainkit module (the standard WASM host module)
	pr, _ := sdk.Publish(k1, ctx, messages.WasmCompileMsg{
		Source: `
			import { _setState, _getState, _setMode, _reply } from "brainkit";
			export function init(): void { _setMode("persistent"); }
			export function handle(payload: usize): void {
				var old = _getState("counter");
				var n = old.length > 0 ? parseInt(old) + 1 : 1;
				_setState("counter", n.toString());
				_reply('{"counter":' + n.toString() + '}');
			}
		`,
		Options: &messages.WasmCompileOpts{Name: "stateful-shard"},
	})
	ch := make(chan []byte, 1)
	...
	case <-ch:
	case <-time.After(30 * time.Second):
		t.Fatal("timeout compiling")
	}
	unsub()

	// Deploy shard
	pr2, _ := sdk.Publish(k1, ctx, messages.WasmDeployMsg{Name: "stateful-shard"})
	ch2 := make(chan []byte, 1)
	unsub2, _ := k1.SubscribeRaw(ctx, pr2.ReplyTo, func(m messages.Message) { ch2 <- m.Payload })
	select {
	case <-ch2:
	case <-time.After(10 * time.Second):
		t.Fatal("timeout deploying shard")
	}
	unsub2()

	k1.Close()

	// Phase 2: Reopen — shard state should persist
	...
	// Module and shard descriptor should be restored
	modules, _ := k2.ListWASMModules()
	found := false
	for _, m := range modules {
		if m.Name == "stateful-shard" {
			found = true
		}
	}
	assert.True(t, found, "stateful shard module should survive restart")
}
