...
// TestFullStack_AMQP_Postgres_PgVector exercises the AMQP transport with
// Postgres storage and pgvector for vector operations.
func TestFullStack_AMQP_Postgres_PgVector(t *testing.T) {
	campaigns.RequirePodman(t)
	infra := campaigns.NewInfra(t,
		campaigns.Transport("amqp"),
		campaigns.Storage("postgres"),
		campaigns.Vector("pgvector"),
