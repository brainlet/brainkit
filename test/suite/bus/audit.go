...
func testAuditQueryAfterDeploy(t *testing.T, env *suite.TestEnv) {
	...
	payload := testutil.PublishAndWait(t, env.Kit, auditmsg.AuditQueryMsg{Category: "deploy"}, 5*time.Second)
	var resp auditmsg.AuditQueryResp
...
func testAuditStatsResponse(t *testing.T, env *suite.TestEnv) {
	payload := testutil.PublishAndWait(t, env.Kit, auditmsg.AuditStatsMsg{}, 5*time.Second)
	var resp auditmsg.AuditStatsResp
	require.NoError(t, json.Unmarshal(payload, &resp))
...
func testAuditPruneWorks(t *testing.T, env *suite.TestEnv) {
	payload := testutil.PublishAndWait(t, env.Kit, auditmsg.AuditPruneMsg{OlderThanHours: 1}, 5*time.Second)
	var resp auditmsg.AuditPruneResp
	require.NoError(t, json.Unmarshal(payload, &resp))
...
func testAuditToolCallRecorded(t *testing.T, env *suite.TestEnv) {
	...
	testutil.PublishAndWait(t, env.Kit, toolmsg.ToolCallMsg{Name: "echo", Input: map[string]any{"message": "audit-test"}}, 5*time.Second)

	// Query audit for tool calls
	payload := testutil.PublishAndWait(t, env.Kit, auditmsg.AuditQueryMsg{Category: "tools"}, 5*time.Second)
	var resp auditmsg.AuditQueryResp
...
func testAuditMetricsGetIncludesBus(t *testing.T, env *suite.TestEnv) {
	...
	payload := testutil.PublishAndWait(t, env.Kit, metricsmod.MetricsGetMsg{}, 5*time.Second)
	var resp metricsmod.MetricsGetResp
	require.NoError(t, json.Unmarshal(payload, &resp))
