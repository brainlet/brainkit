...
func testConcurrentPublish50(t *testing.T, _ *suite.TestEnv) {
	...
	unsub, err := env.Kernel.SubscribeRaw(ctx, "incoming.concurrent.suite", func(m messages.Message) {
		received.Add(1)
	})
	...
	for i := 0; i < 50; i++ {
		...
		go func(n int) {
			defer wg.Done()
			env.Kernel.PublishRaw(ctx, "incoming.concurrent.suite", json.RawMessage(fmt.Sprintf(`{"n":%d}`, n)))
		}(i)
	}
	wg.Wait()
...
func testLargePayload100KB(t *testing.T, _ *suite.TestEnv) {
	...
	_, err := env.Kernel.Deploy(ctx, "big-msg-suite.ts", `
	...
	`)
	require.NoError(t, err)
	defer env.Kernel.Teardown(ctx, "big-msg-suite.ts")
...
func testDottedTopicNames(t *testing.T, _ *suite.TestEnv) {
	...
	_, err := env.Kernel.Deploy(ctx, "my.dotted.agent.suite.ts", `
		bus.on("ask", function(msg) { msg.reply({dotted: true}); });
	`)
	require.NoError(t, err)
	defer env.Kernel.Teardown(ctx, "my.dotted.agent.suite.ts")
...
func testDeployHandlerCall(t *testing.T, _ *suite.TestEnv) {
	...
	_, err := env.Kernel.Deploy(ctx, "backend-handler-suite.ts", `
	...
	`)
	require.NoError(t, err)
	defer env.Kernel.Teardown(ctx, "backend-handler-suite.ts")
...
func testPublishReply(t *testing.T, _ *suite.TestEnv) {
	...
	_, err := env.Kernel.Deploy(ctx, "backend-reply-suite.ts", `
		bus.on("ping", function(msg) { msg.reply({backend: "works"}); });
	`)
	require.NoError(t, err)
	defer env.Kernel.Teardown(ctx, "backend-reply-suite.ts")
...
func testErrorCodeOnBus(t *testing.T, _ *suite.TestEnv) {
	...
	pr, err := sdk.Publish(env.Kernel, ctx, messages.ToolCallMsg{Name: "ghost-backend-tool-suite"})
	require.NoError(t, err)

	ch := make(chan json.RawMessage, 1)
	unsub, err := env.Kernel.SubscribeRaw(ctx, pr.ReplyTo, func(m messages.Message) {
...
func testTransportCompliancePublishSubscribe(t *testing.T, env *suite.TestEnv) {
	...
	unsub, err := env.Kernel.SubscribeRaw(ctx, "test.compliance.topic", func(msg messages.Message) {
		received <- msg.Payload
	})
	require.NoError(t, err)
	defer unsub()

	env.Kernel.PublishRaw(ctx, "test.compliance.topic", []byte(`{"hello":"world"}`))
...
func testTransportComplianceCorrelationID(t *testing.T, env *suite.TestEnv) {
	...
	unsub, err := env.Kernel.SubscribeRaw(ctx, "corr.topic.suite", func(msg messages.Message) {
		received <- msg.Metadata["correlationId"]
	})
	require.NoError(t, err)
	defer unsub()

	env.Kernel.PublishRaw(ctx, "corr.topic.suite", []byte(`{}`))
...
func testTransportComplianceDottedTopics(t *testing.T, env *suite.TestEnv) {
	...
	unsub, err := env.Kernel.SubscribeRaw(ctx, "ai.generate.suite", func(msg messages.Message) {
		received <- true
	})
	require.NoError(t, err)
	defer unsub()

	env.Kernel.PublishRaw(ctx, "ai.generate.suite", []byte(`{"model":"test"}`))
