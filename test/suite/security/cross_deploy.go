...
func testXDeployTeardownAnother(t *testing.T, env *suite.TestEnv) {
	...
	_, err := k.Deploy(ctx, "victim-svc-sec.ts", `
		bus.on("ping", function(msg) { msg.reply({alive: true}); });
	`)
	require.NoError(t, err)

	_, err = k.Deploy(ctx, "attacker-teardown-sec.ts", `
		var result = "UNKNOWN";
		try {
	...
	attackerResult, _ := k.EvalTS(ctx, "__atk_td.ts", `return String(globalThis.__module_result || "");`)
...
func testXDeployReplyImpersonation(t *testing.T, env *suite.TestEnv) {
	...
	_, err := k.Deploy(ctx, "slow-a-sec.ts", `
	...
	_, err = k.Deploy(ctx, "impersonator-b-sec.ts", `
...
func testXDeployUnregisterAlienTool(t *testing.T, env *suite.TestEnv) {
	...
	attackerResult, _ := k.EvalTS(ctx, "__thief.ts", `return String(globalThis.__module_result || "");`)
...
func testXDeployStealOutput(t *testing.T, env *suite.TestEnv) {
	...
	_, err := k.Deploy(ctx, "secret-output-sec.ts", `
		output({secretKey: "sk-12345-super-secret", apiToken: "tok-9999"});
	`)
	require.NoError(t, err)

	_, err = k.Deploy(ctx, "steal-output-sec.ts", `
		var stolen = "FAILED";
		try {
	...
	result, _ := k.EvalTS(ctx, "__steal.ts", `return String(globalThis.__module_result || "");`)
...
func testXDeployMailboxEavesdrop(t *testing.T, env *suite.TestEnv) {
	...
	_, err := k.Deploy(ctx, "private-svc-sec.ts", `
...
func testXDeployAgentRegistrationRace(t *testing.T, env *suite.TestEnv) {
	...
	_, err := k.Deploy(ctx, "agent-a-sec.ts", `
		kit.register("agent", "shared-bot-sec", {});
		output("registered-by-a");
	`)
	require.NoError(t, err)

	_, err = k.Deploy(ctx, "agent-b-sec.ts", `
		var result = "UNKNOWN";
		try {
			kit.register("agent", "shared-bot-sec", {});
	...
	result, _ := k.EvalTS(ctx, "__agent_race.ts", `return String(globalThis.__module_result || "");`)
...
func testXDeployCreateToolMonkeyPatch(t *testing.T, env *suite.TestEnv) {
	...
	_, err := k.Deploy(ctx, "patch-createtool-sec.ts", `
		try {
	...
	_, err = k.Deploy(ctx, "innocent-tool-sec.ts", `
...
func testXDeploySendToCrafted(t *testing.T, env *suite.TestEnv) {
	...
	_, err := k.Deploy(ctx, "victim-process-sec.ts", `
	...
	_, err = k.Deploy(ctx, "attacker-send-sec.ts", `
	...
	assert.True(t, k.Alive(ctx), "kernel should survive crafted sendTo payload")
}
...
func testXDeploySelfRedeploy(t *testing.T, env *suite.TestEnv) {
	...
	_, err := k.Deploy(ctx, "self-redeploy-sec.ts", `
		var result = "UNKNOWN";
		try {
...
func testXDeployWorkflowEscalation(t *testing.T, env *suite.TestEnv) {
	...
	_, err := k.Deploy(ctx, "wf-escalate-sec.ts", `
		var result = "UNKNOWN";
		try {
	...
	result, _ := k.EvalTS(ctx, "__wf_esc.ts", `return String(globalThis.__module_result || "");`)
