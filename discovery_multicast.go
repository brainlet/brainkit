...
// MulticastDiscovery uses UDP multicast for zero-config peer discovery on a LAN.
...
func (d *MulticastDiscovery) Resolve(name string) (string, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
...
func (d *MulticastDiscovery) Browse() ([]Peer, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
...
func (d *MulticastDiscovery) Register(self Peer) error {
	d.mu.Lock()
	d.self = &self
	d.mu.Unlock()
...
func (d *MulticastDiscovery) announce(self Peer) error {
	addr, err := net.ResolveUDPAddr("udp4", multicastAddr)
	if err != nil {
		return err
	}
...
func (d *MulticastDiscovery) listenLoop(ctx context.Context) {
...
func (d *MulticastDiscovery) parseAnnouncement(msg string) {
