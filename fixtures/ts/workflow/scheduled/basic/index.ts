...
const scheduledWorkflow = createWorkflow({
  ...
  schedule: {
    ...
    requestContext: { source: "schedule-fixture" },
    metadata: { fixture: "scheduled-basic" },
  },
})
  .then(scheduledStep)
  .commit();
...
try {
  ...
  await waitFor("scheduled-workflow-execution", () => executions[0]);
  ...
  observed = {
    ...
    nextFireAdvanced: Number(updatedSchedule?.nextFireAt || 0) > dueAt,
