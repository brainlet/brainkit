...
output({
  ...
  workspaceToolsPrefix: WORKSPACE_TOOLS_PREFIX === "mastra_workspace",
  workspaceToolsReadFile: WORKSPACE_TOOLS?.FILESYSTEM?.READ_FILE === "mastra_workspace_read_file",
  workspaceToolsExecuteCommand:
    WORKSPACE_TOOLS?.SANDBOX?.EXECUTE_COMMAND === "mastra_workspace_execute_command",
  ...
  resolveToolConfig: isFn(resolveToolConfig),
  ...
  requireWorkspace: isFn(requireWorkspace),
  requireFilesystem: isFn(requireFilesystem),
  requireSandbox: isFn(requireSandbox),
  mastraBrowser: isFn(MastraBrowser),
  browserContextProcessor: isFn(BrowserContextProcessor),
  agentChannels: isFn(AgentChannels),
  chatChannelProcessor: isFn(ChatChannelProcessor),
  mastraStateAdapter: isFn(MastraStateAdapter),
  chainFormatters: isFn(chainFormatters),
  assignTaskIds: isFn(assignTaskIds),
  defaultDisplayState: isFn(defaultDisplayState),
  defaultOMProgressState: isFn(defaultOMProgressState),
  parseSubagentMeta: isFn(parseSubagentMeta),
  askUserTool: isDef(askUserTool),
  submitPlanTool: isDef(submitPlanTool),
  taskWriteTool: isDef(taskWriteTool),
  taskUpdateTool: isDef(taskUpdateTool),
  taskCompleteTool: isDef(taskCompleteTool),
  taskCheckTool: isDef(taskCheckTool),
