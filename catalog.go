...
func commandCatalog() *commandRegistry {
	commandCatalogOnce.Do(func() {
		specs := []commandSpec{
			...
			// ── Workflows (Mastra engine via JS eval, delegates to Mastra APIs directly) ──
			...
			kernelCommand(func(ctx context.Context, kernel *Kernel, req messages.WorkflowRunsMsg) (*messages.WorkflowRunsResp, error) {
			...
			kernelCommand(func(ctx context.Context, kernel *Kernel, req messages.WorkflowRestartMsg) (*messages.WorkflowRestartResp, error) {
				script := fmt.Sprintf(`
					var entry = globalThis.__kit_registry.get("workflow", %q);
					if (!entry || !entry.ref) throw new BrainkitError("workflow not found: " + %q, "NOT_FOUND");
					var wf = entry.ref;
