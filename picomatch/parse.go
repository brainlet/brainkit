...
type Token struct {
	...
	Value       string
	Output      string
	OutputSet   bool // true when Output was explicitly set (even if empty)
	OutputIndex int
	TokensIndex int
	Prev        *Token
	Suffix      string
...
// This mirrors picomatch.parse(), which disables fastpaths.
func Parse(input string, opts *Options) *ParseState {
	parsedOpts := &Options{Fastpaths: boolPtr(false)}
	if opts != nil {
		cloned := *opts
		cloned.Fastpaths = boolPtr(false)
		parsedOpts = &cloned
	}
	return parseInternal(input, parsedOpts)
}

// parseInternal mirrors lib/parse.js directly.
// JS source: parse.js lines 55-991
func parseInternal(input string, opts *Options) *ParseState {
	if opts == nil {
		opts = &Options{}
	}
	...
	advance := func() string {
		...
		if state.Index < inputLen {
			if input[state.Index] < utf8.RuneSelf {
				return string(input[state.Index])
			}
			_, size := utf8.DecodeRuneInString(input[state.Index:])
			ch := input[state.Index : state.Index+size]
			state.Index += size - 1
			return ch
		}
		return ""
	...
	tokenOutput := func(tok *Token) string {
		if tok.OutputSet || tok.Output != "" {
			return tok.Output
		}
		return tok.Value
	}
	trimOutput := func(n int) {
		if n <= 0 {
			return
		}
		if n >= len(state.Output) {
			state.Output = ""
			return
		}
		state.Output = state.Output[:len(state.Output)-n]
	}

	appendTok := func(tok *Token) {
		state.Output += tokenOutput(tok)
		consume(tok.Value, 0)
	}
	...
	push = func(tok *Token) {
		...
		if tok.Value != "" || tok.OutputSet || tok.Output != "" {
			appendTok(tok)
		}
		...
		if token.Type == "negate" {
			...
			if strings.Contains(token.Inner, "*") {
				...
				if rest != "" && matchStr(`^\.[^\\/\.]+$`, rest) {
					exprOpts := *opts
					exprOpts.Fastpaths = boolPtr(false)
					expr := parseInternal(rest, &exprOpts)
					output = ")" + expr.Output + ")" + extglobStar + ")"
					token.Close = output
				}
			}
	...
	for !eos() {
		...
		if value == "+" {
			...
			if (prev != nil && prev.Value == "(") || (opts.Regex != nil && !*opts.Regex) {
		...
		if value == "@" {
			if !opts.Noextglob && peek(1) == "(" && peek(2) != "?" {
				push(&Token{Type: "at", Extglob: true, Value: value, Output: "", OutputSet: true})
				continue
			}
		...
		if prev.Type == "star" {
			...
			if opts.Bash && (!isStart || (len(rest) > 0 && rest[0] != '/')) {
				push(&Token{Type: "star", Value: value, Output: "", OutputSet: true})
				continue
			}
			...
			if prior.Type == "slash" && prior.Prev.Type != "bos" && !afterStar && eos() {
				trimOutput(len(prior.Output + prev.Output))
			...
			if prior.Type == "slash" && prior.Prev.Type != "bos" && len(rest) > 0 && rest[0] == '/' {
				...
				push(&Token{Type: "slash", Value: "/", Output: "", OutputSet: true})
				continue
			}
		...
		if prev != nil && (prev.Type == "bracket" || prev.Type == "paren") && opts.Regex != nil && *opts.Regex {
			tok.Output = value
			push(tok)
			continue
		}
