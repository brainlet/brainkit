// Package postgres provides the PostgreSQL audit store without linking other
// audit store backend drivers.
package postgres
...
// Store persists audit events in a Postgres database using the shared
...
// New opens a Postgres audit store. Creates the audit_events
// table if missing.
func New(connStr string) (*Store, error) {
	...
	return &Store{db: db, queries: pggen.New(db)}, nil
}

func (s *Store) Record(event auditpkg.Event) {
	if event.ID == "" {
...
func (s *Store) Query(q auditpkg.Query) ([]auditpkg.Event, error) {
	limit := int32(q.Limit)
	if limit <= 0 {
		limit = 100
	}
...
func (s *Store) CountByCategory() (map[string]int64, error) {
