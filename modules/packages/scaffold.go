...
func ScaffoldPackage(dir, name, entry, source string, opts ...ScaffoldOptions) error {
	if dir == "" {
		return fmt.Errorf("packages.ScaffoldPackage: dir is required")
	}
	if name == "" {
		return fmt.Errorf("packages.ScaffoldPackage: name is required")
	}
	if entry == "" {
		entry = "index.ts"
	}
	...
	if _, err := os.Stat(dir); err == nil && !opt.Overwrite {
		...
		if readErr == nil && len(entries) > 0 {
			return fmt.Errorf("packages.ScaffoldPackage: %s already exists and is not empty; pass ScaffoldOptions{Overwrite: true} to replace", dir)
		}
	}
	if err := os.MkdirAll(filepath.Join(dir, "types"), 0o755); err != nil {
		return fmt.Errorf("packages.ScaffoldPackage: mkdir: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("packages.ScaffoldPackage: marshal manifest: %w", err)
	}
	...
	for path, content := range opt.Extra {
		if _, owned := files[path]; owned {
			return fmt.Errorf("packages.ScaffoldPackage: Extra tried to overwrite scaffold-owned file %q; use ScaffoldOptions top-level fields instead", path)
		}
		files[path] = content
	}
	for path, content := range files {
		...
		if err := os.MkdirAll(filepath.Dir(full), 0o755); err != nil {
			return fmt.Errorf("packages.ScaffoldPackage: mkdir %s: %w", filepath.Dir(full), err)
		}
		if err := os.WriteFile(full, []byte(content), 0o644); err != nil {
			return fmt.Errorf("packages.ScaffoldPackage: write %s: %w", path, err)
		}
	}
	return nil
}
