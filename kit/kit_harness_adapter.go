...
func (k *Kit) InitHarness(cfg harness.HarnessConfig) (*harness.Harness, error) {
	return harness.Init(k, cfg)
}

// --- harness.Runtime implementation ---

func (k *Kit) BridgeIsEvalBusy() bool {
	return k.bridge.IsEvalBusy()
}

func (k *Kit) BridgeEval(filename, code string) (*quickjs.Value, error) {
	return k.bridge.Eval(filename, code)
}

func (k *Kit) BridgeEvalOnJSThread(filename, code string) (string, error) {
	return k.bridge.EvalOnJSThread(filename, code)
}

func (k *Kit) BridgeContext() *quickjs.Context {
	return k.bridge.Context()
}

func (k *Kit) BridgeGoContext() context.Context {
