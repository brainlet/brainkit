...
type Kit struct {
	...
	discovery discovery.Provider // optional peer discovery
...
func New(cfg Config) (*Kit, error) {
	...
	case "multicast":
		disc, err := discovery.NewMulticast(cfg.Network.Discovery.ServiceName)
		if err != nil {
	...
	case "static":
		if len(cfg.Network.Peers) > 0 {
			k.discovery = discovery.NewStatic(cfg.Network.Peers)
		}
	}
	...
	if k.discovery != nil && cfg.Network.Listen != "" && cfg.Name != "" {
		k.discovery.Register(discovery.Peer{
			Name:    cfg.Name,
