// Ported from: packages/ai/src/text-stream/pipe-text-stream-to-response.test.ts
package textstream
...
func TestPipeTextStreamToResponse(t *testing.T) {
	t.Run("should write to response with correct headers and encoded stream", func(t *testing.T) {
		textStream := make(chan string, 1)
		textStream <- "test-data"
		close(textStream)

		recorder := httptest.NewRecorder()
		PipeTextStreamToResponse(PipeTextStreamToResponseOptions{
			Response: recorder,
			Status:   200,
