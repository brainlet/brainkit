...
// Interrupt asks a running eval to stop and cancels Go-backed async work.
// It does not free the runtime. Callers use this before waiting for handler
// shutdown so router drains are not blocked by runaway JavaScript.
...
// Skips if an EvalJS/EvalAsync is already active (those process jobs
