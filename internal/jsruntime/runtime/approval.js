...
(function() {
  ...
  function copyAgentOptions(agentOptions) {
    var copied = {};
    for (var key in agentOptions) {
      copied[key] = agentOptions[key];
    }
    return copied;
  }
  ...
  globalThis.__kit_generateWithApproval = async function(agent, promptOrMessages, options) {
    ...
    while (result && result.finishReason === "suspended" && result.runId) {
      ...
      var resumeOpts = copyAgentOptions(agentOptions);
      resumeOpts.runId = result.runId;
      resumeOpts.toolCallId = result.suspendPayload && result.suspendPayload.toolCallId;
    ...
    if (result && Array.isArray(result.toolResults)) {
      result.toolResults = result.toolResults.map(normalizeToolResult);
    }
    return result;
  };
})();
